...
define("SSLCZ_STORE_ID", config('sslcommerze.STORE_ID',null));
define("SSLCZ_STORE_PASSWD", config('sslcommerze.STORE_PASSWORD',null));
$app_env = config('sslcommerze.APP_ENV') == 'production' ? false : true;
# IF SANDBOX TRUE, THEN IT WILL CONNECT WITH SSLCOMMERZ SANDBOX (TEST) SYSTEM
define("SSLCZ_IS_SANDBOX", $app_env);

# IF BROWSE FROM LOCAL HOST, KEEP true
define("SSLCZ_IS_LOCAL_HOST", $app_env);
...
class SSLCommerz
{
    ...
    public function paymentInfo($request){
        ...
        $subs_id = $request['subscription_id'];
        $media_id = 3;
        ...
        $server_name=config('sslcommerze.APP_URL')."/";
        $post_data = array();
        
        if(!$transaction){
        Log::error("New Transection not craeted");
        return ;
        }
        # Mandatory PARAMETERS
        $post_data['total_amount'] = $transaction->amount; # You cant not pay less than 10
        $post_data['currency'] = $transaction->currency;
        ...
        if($post_data['total_amount'] && $post_data['currency'] &&
            $post_data['tran_id'] && $post_data['success_url'] && $post_data['fail_url'] && $post_data['cancel_url'] && 
            $post_data['cus_name'] && $post_data['cus_email'] && $post_data['cus_phone'])
            {
                ...
                $post_data['value_b'] = $subs_id;
                $post_data['value_c'] = $media_id;
                $post_data['value_d'] = "";
                return $post_data;
        }else{
             Log::info("Don't find out mandatory field.");
        }
    ...
    {
        Log::info("Requested data : -",["data"=>$post_data]);
            ...
            if ($load_sslc) {
                if (isset($this->sslc_data['status']) && $this->sslc_data['status'] == 'SUCCESS') {
                    if (!$get_pay_options) {
                        ...
                        if (isset($this->sslc_data['sessionkey'])){
                            Transaction::updateSessionKey($post_data['tran_id'],$this->sslc_data['sessionkey']);
                        }
                    ...
                    Log::info("Invalid Credential! For this transection  :".$post_data['tran_id'].' SSLCommerz status response : '.$this->sslc_data['status']);
                    $this->error = "Invalid Credential!";
                }

            } else {
                Log::info("SSlcommerz API Connectivity Issue.! For this transection  :".$post_data['tran_id']);
                $this->error = "Connectivity Issue. Please contact your sslcommerz manager";
            }
        } else {
            Log::info("Please provide a valid information list about transaction.! For this transection  :".$post_data['tran_id']);
            $this->error = "Please provide a valid information list about transaction with transaction id, amount, success url, fail url, cancel url, store id and pass at least";
        }
        return $result = array(
            'status' => 'FAILED',
            'error' => $this->error,
    ...
    public function orderValidate($trx_id = '', $amount = 0, $currency = "BDT", $post_data)
    {
        if ($post_data == '' && $trx_id == '' && !is_array($post_data)) {
            Log::info('Does not provide valid transaction ID and post request data for this transection :-'.$trx_id);
            ...
            if ($validation) {
                Log::info('Validation successfull for transaction :-'.$trx_id);    
                return true;
            } else {
                Log::info('Validation fail for transaction :-'.$trx_id); 
                return false;
            }
    ...
    protected function sendRequest($data)
    {
        ...
        if ($code == 200 && !(curl_errno($handle))) {
            ...
            curl_close($handle);
            Log::error('FAILED TO CONNECT WITH SSLCOMMERZ API FOR '.$data['tran_id'].' For THIS TRANSECTION');
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            $post_data['store_id'] =  config('sslcommerze.STORE_ID',null);
            $post_data['store_pass'] =config('sslcommerze.STORE_PASSWORD',null);

            if ($this->SSLCOMMERZ_hash_varify($this->store_pass, $post_data)) {

                $val_id = urlencode($post_data['val_id']);
                $store_id = urlencode(config('sslcommerze.STORE_ID',null));
                $store_passwd = urlencode(config('sslcommerze.STORE_PASSWORD',null));
                ...
                if (SSLCZ_IS_LOCAL_HOST)
                {
                    curl_setopt($handle, CURLOPT_SSL_VERIFYHOST, false);
                    curl_setopt($handle, CURLOPT_SSL_VERIFYPEER, false);
                    Log::info('SSLCommerz host environment :- '.SSLCZ_IS_LOCAL_HOST);
                    ...
                    if ($status == "VALID" || $status == "VALIDATED") {
                        if ($merchant_trans_currency == "BDT") {
                            if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $amount) < 1) && trim($merchant_trans_currency) == trim('BDT')) {
                                ...
                                # DATA TEMPERED
                                Log::info("Data has been tempered for :- ".$tran_id);
                        ...
                        # FAILED TRANSACTION
                        Log::info("Validation Failed For Transaction  :- ".$tran_id);
                        $this->error = "Failed Transaction";
                        return false;
                    }
                } else {
                    # Failed to connect with SSLCOMMERZ
                    Log::info("Faile to connect with SSLCOMMERZ Transaction  :- ".$post_data['tran_id']);
                    $this->error = "Faile to connect with SSLCOMMERZ";
                    return false;
                }
            } else {
                # Hash validation failed
                Log::info("SSLCOMMERZ  Hash validation failed,  Transaction  :- ".$post_data['tran_id']);
                $this->error = "Hash validation failed";
                return false;
            }
        } else {
            # INVALID DATA
            Log::info(" Validate Data messing for ,  Transaction  :- ".$post_data['tran_id']);
    ...
    protected function SSLCOMMERZ_hash_varify($store_passwd = "", $post_data)
    {
        ...
        if (isset($post_data) && isset($post_data['verify_sign']) && isset($post_data['verify_key'])) {
            ...
            if (md5($hash_string) == $post_data['verify_sign']) {
                ...
                Log::info('Verification signature not matched. Transection id  :- '.$post_data['tran_id']);
                $this->error = "Verification signature not matched";
                return false;
            }
        } else {
            Log::info('Required data mission. ex: verify_key, verify_sign. Transection id  :- '.$post_data['tran_id']);
